...
def update_cart(item_code, qty, additional_notes=None, with_items=False):
	...
	if cint(with_items):
		...
		cart_item_template = "templates/includes/cart/cart_items_b2c.html"
		if(get_customer_type()=="Company"):
			cart_item_template = "templates/includes/cart/cart_items_b2b.html"

		return {
			"items": frappe.render_template(
				cart_item_template, context
			),
...
def remove_item(q_item_name, with_items=False):
	...
	quotation_items = quotation.get("items", {"name": ["!=", q_item_name]})
	if quotation_items:
		quotation.set("items", quotation_items)
	else:
		empty_card = True
